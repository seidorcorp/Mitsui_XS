...
function xml2toJsonConsultarCliente(xml){
	 var parser 		= new $.util.SAXParser();
	  ...
	  var lsObject=[];
	  var settings 		= {};
	  settings.BOCitasPorLocalExcepcion			= { consider: true, parent: null, type: 'array' };
	  settings.SAP_UUID 				= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'SAP_UUID' };
	  settings.zItem 					= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'zItem' };
	  settings.zIDCentro 				= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'zIDCentro' };
	  settings.zHoraInicio 				= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'zHoraInicio' };
	  settings.zTope 					= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'zTope' };
	  settings.zDuracion 				= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'zDuracion' };
	  settings.zFechaInicioValidez 		= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'zFechaInicioValidez' };
	  settings.zFechaFinValidez 		= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'zFechaFinValidez' };
	  settings.zEstado 					= { consider: true, parent: 'BOCitasPorLocalExcepcion', type: 'property', mask:'zEstado' };
	  ...
	  parser.startElementHandler = function(name, attrs){
		  ...
		  parser.characterDataHandler = function(value){
			  ...
			  if(lastSetting !== undefined && lastSetting.type === 'property' && lastSetting.consider === true){
				  lastElement["BOCitasPorLocalExcepcion"][lsObject.length - 1][lastSetting.mask]=value;
			  }
