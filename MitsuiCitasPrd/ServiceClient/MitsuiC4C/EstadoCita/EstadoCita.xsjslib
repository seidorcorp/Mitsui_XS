...
var destination_package = config.getText("package.project")+".ServiceClient.MitsuiC4C.EstadoCita";
var utils 				= $.import("MitsuiCitasPrd.ServiceClient.MitsuiC4C.EstadoCita","Utils");
 ...
 * @update
 */
function CambiarEstadoCitaC4c(oParam){
		var responseClient;
		var oRequest = {};
		oRequest.oData = {};
		var bodyJson ='';
		...
		try{
				   ...
				   '<soap:Header/>' +
			   			'<soap:Body>' +
					   		  '<glob:AppointmentActivityBundleMaintainRequest_sync_V1>' +
						            ...
						            '<y6s:zEstadoCita listID="" listVersionID="" listAgencyID="">4</y6s:zEstadoCita>' +
								'</AppointmentActivity>' +
						      '</glob:AppointmentActivityBundleMaintainRequest_sync_V1>'+
		      			'</soap:Body>'+
		   			'</soap:Envelope>';
			...
			if(cita.AppointmentActivity !== undefined 
				...
				oResponse.sMessage 	= bundle.getText("msj.cita.estado.error",[cita.Log.Item[0].Note + " --- Trama Enviada: " + requestGenerica + "---Trama Respuesta: " + bodyJson]);
			}
			...
			oResponse.sMessage 	= bundle.getText("msj.idt10",["CambiarEstadoCitaC4c",e.toString() + " --- Trama Enviada: " + requestGenerica + "---Trama Respuesta: " + bodyJson]);
		}
		return oResponse;
}
