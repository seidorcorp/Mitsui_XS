...
var productoCnBusiness 	= $.import("MitsuiCitasPrd.Business.Producto","ProductoCnBusiness");
...
function validarParamConsultarProductos(){
	var oResponse = {};
...
function consultarProducto(){
	...
	try{
		...
		var buscarProductosxFiltroResponse;

		switch(Accion) {
			case 'ConsultarProducto':
				var validarParamConsultarProductosResponse = validarParamConsultarProductos();
				if(validarParamConsultarProductosResponse.iCode === parseInt(bundle.getText("code.idf1"), 10)){
					oParam.oAuditRequest = oAuditoriaRequest;
					oParam.oData = oDataRequest;
					buscarProductosxFiltroResponse = productoCnBusiness.buscarProductosxFiltro(oParam);
					if(buscarProductosxFiltroResponse.iCode === parseInt(bundle.getText("code.idf1"), 10)){
						utils.sendResponse(
								sIdTransaccion,
								parseInt(bundle.getText("code.idf1"), 10), 
								bundle.getText("msj.idf1"), 
								buscarProductosxFiltroResponse.oData );
					}else{
						utils.sendResponseError(
								sIdTransaccion,
								buscarProductosxFiltroResponse.iCode, 
								buscarProductosxFiltroResponse.sMessage 
								);
					}
				}else{
					utils.sendResponseError(
							sIdTransaccion,
							validarParamConsultarProductosResponse.iCode, 
							validarParamConsultarProductosResponse.sMessage
							);
				}
				break;
